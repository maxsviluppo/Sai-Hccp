    ...
    <div class="space-y-8 pb-10">
        ...
        <div class="bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-600 p-8 rounded-3xl shadow-xl border border-violet-500/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
            <div class="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
                <i class="fa-solid fa-broom text-9xl text-white"></i>
            </div>
            <div class="relative z-10">
                ...
                <p class="text-violet-100 text-sm mt-2 font-medium ml-1">Registro attività di pulizia e manutenzione</p>
            </div>
            <div class="relative z-10 flex flex-col gap-2">
                <div class="flex items-center gap-3 bg-white/10 backdrop-blur-md px-4 py-2 rounded-xl border border-white/20">
                    <i class="fa-solid fa-spray-can text-white text-lg"></i>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @for (check of checks(); track check.id) {
                <div class="bg-white p-6 rounded-2xl shadow-sm border-2 transition-all duration-300 group"
                     ...
                     [class.border-violet-200]="!check.checked" [class.border-violet-500]="check.checked"
                    ...
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300"
                            ...
                            <i class="fa-solid text-2xl" [class.fa-check-circle]="check.checked" [class.fa-spray-can]="!check.checked"></i>
                        </div>
...
export class CleaningMaintenanceViewComponent {
    ...
    checks = signal<CheckItem[]>([
        { id: 'equip', label: 'ATTREZZATURE', checked: false },
        { id: 'work-env', label: 'AMBIENTI DI LAVORO', checked: false },
        { id: 'toilets', label: 'SERVIZI IGIENICI', checked: false },
        { id: 'changing-room', label: 'SPOGLIATOIO', checked: false },
        { id: 'storage', label: 'DEPOSITO / MAGAZZINO', checked: false }
    ]);
