...
@Component({
    ...
    <div class="space-y-8 pb-10">
        ...
        <div class="bg-gradient-to-r from-violet-600 via-indigo-600 to-purple-600 p-8 rounded-3xl shadow-xl border border-violet-500/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
            <div class="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
                <i class="fa-solid fa-vial-virus text-9xl text-white"></i>
            </div>
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-center gap-4 mt-2">
                    <p class="text-violet-100 text-sm font-medium ml-1">Analisi di laboratorio e tamponi superficiali</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @for (check of checks(); track check.id) {
                <div class="bg-white p-6 rounded-2xl shadow-sm border-2 transition-all duration-300 group"
                     ...
                     [class.border-violet-100]="!check.checked" [class.border-violet-500]="check.checked"
                     [class.bg-violet-50]="check.checked" [class.shadow-lg]="check.checked" [class.shadow-violet-200/50]="check.checked"
                    ...
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300"
                             [class.bg-violet-500]="check.checked" [class.text-white]="check.checked" [class.shadow-lg]="check.checked"
                             [class.shadow-violet-200]="check.checked" [class.bg-slate-100]="!check.checked" [class.text-slate-400]="!check.checked">
                            <i class="fa-solid text-2xl" [class.fa-check-circle]="check.checked" [class.fa-vial-circle-check]="!check.checked"></i>
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2">
                                @if (check.checked) {
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-violet-500 text-white text-xs font-bold rounded-full">
        ...
        @if (showStandardInfo()) {
            <div class="fixed inset-0 z-[110] flex items-center justify-center p-4">
                ...
                <div class="relative bg-white w-full max-w-md max-h-[90vh] rounded-[40px] shadow-2xl overflow-hidden animate-slide-up border border-slate-100 flex flex-col">
                    <div class="p-8 bg-gradient-to-br from-violet-700 to-indigo-900 text-white relative flex-shrink-0">
                        ...
                        <div class="relative z-10">
                            <h3 class="text-2xl font-black mb-1">Microbiologia</h3>
                            <p class="text-violet-200 text-[10px] font-black uppercase tracking-[0.2em]">Standard HACCP Pro</p>
                        </div>
                    </div>
                    ...
                    <div class="p-8 pt-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-violet-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-calendar-check text-xs"></i> 01. Frequenza
                            </h4>
                            <div class="bg-violet-50/50 p-5 rounded-2xl border border-violet-100">
                                <p class="text-xs text-slate-700 leading-relaxed font-medium">
                                    Le analisi devono seguire il calendario previsto dal piano di campionamento aziendale.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-violet-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-microscope text-xs"></i> 02. Accreditamento
                            </h4>
                            <div class="bg-violet-50/50 p-5 rounded-2xl border border-violet-100">
                                <p class="text-xs text-slate-700 leading-relaxed font-medium">
                                    I campioni devono essere analizzati esclusivamente da laboratori esterni certificati e accreditati.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            <h4 class="text-[10px] font-black text-violet-600 uppercase tracking-widest flex items-center gap-2">
                                <i class="fa-solid fa-triangle-exclamation text-xs"></i> 03. Superamento Limiti
                            </h4>
                            <div class="bg-violet-50/50 p-5 rounded-2xl border border-violet-100 text-[10px] font-bold text-slate-500 italic">
                                In caso di esito sfavorevole, attivare immediatamente la procedura di sanificazione straordinaria e ripetere il test.
                            </div>
                        </div>
                    </div>
